...
macro "Spheroid Detection - Merged TIF Analysis" {
    ...
    detectSpheroids(dir, list, filter);
}

function detectSpheroids(dir, list, filter) {
    for (i = 0; i < list.length; i++) {
        ...
        if (startsWith(list[i], "Merged_") && endsWith(list[i], filter)) {
            ...
            run("Cellpose Advanced", "diameter=100 cellproba_threshold=0.0 flow_threshold=0.4 anisotropy=1.0 diam_threshold=80.0 model=cyto2 nuclei_channel=0 cyto_channel=1 dimensionmode=2D stitch_threshold=-1.0 omni=false cluster=false additional_flags=");
